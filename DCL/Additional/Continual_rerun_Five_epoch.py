        ...
        for epoch in range(num_epochs):
            ...
            for sample in dataloaders:
                ...
                for _ in range(K):
                    ...
                    # V(k+1)-V(k)
                    Total_loss = 0.1*criterion(V_k_plus_1, V_k)
                    self.optimizer.zero_grad()
...
for i in range(reruns):
    ...
    for  samp_num in range(total_samples):
        ...
        dataloaders_exp = DataLoader(dataset_exp, batch_size = 25, shuffle=True, num_workers=1)
        loss = model.backward(num_epochs = 5, K = 5, batch_size = 8,\
        dataloaders = dataloaders, dataloaders_exp = dataloaders_exp, criterion = torch.nn.MSELoss())
        
        # Evaluation section
        curr_error = model.return_score_current(samp_num)
        prev_error, _ = model.return_score( (samp_num+1))
        # print("Sample Number ", samp_num, "The Current and Previous errors are", curr_error, prev_error)
        result_array_current.append(curr_error)
        result_array_previous.append( prev_error)   

    print("run number #####", i, "Time elapsed for the current run",(time.time()-star_time))
    _, all_errors[i,:]= model.return_score( (samp_num+1))
    # Save the files before going to the next run
    Result_array_previous[i,:] = np.array(result_array_previous).reshape([-1])
    Result_array_current[i,:] = np.array(result_array_current).reshape([-1])
    del model
    del dataloaders
    del dataloaders_exp


np.savetxt('/home/kraghavan/Projects/Continual/Incremental_Sine/Result_Reruns/result_DCL_training_constant_training_samp_num_wo_current_error_Five_epoch'+ str(i)+'.csv',\
Result_array_current)
np.savetxt('/home/kraghavan/Projects/Continual/Incremental_Sine/Result_Reruns/result_DCL_training_constant_training_samp_num_wo_PREV_error_Five_epoch'+ str(i)+'.csv',\
Result_array_previous)
np.savetxt('/home/kraghavan/Projects/Continual/Incremental_Sine/Result_Reruns/result_DCL_all_error_samp_num_wo_error_Five_epoch'+ str(i)+'.csv',\
all_errors )