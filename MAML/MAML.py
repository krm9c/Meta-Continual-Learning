...
for i in range(total_runs):
    ...
    for  samp_num in range(total_samples):
        ...
        dataloaders_experience = DataLoader(dataset_experience, batch_size=8, shuffle=True, num_workers=1)
        for epoch in range(num_epochs):
            # First the Meta update
            for sample in dataloaders_experience:
                ...
                # MAML meta update is equivalent to two succesive steps of update 
                ...
                loss.backward(create_graph=True)
                optimizer_meta.step()
            # Next, the update for the current task
            for sample in dataloaders:
                x = sample['x'].float()
                y = sample['y'].float()
                x, y = x.to(device), y.to(device)
...
plt.savefig("/home/kraghavan/Projects/Continual/MAML/Results_paper/MAML_training_previous_tasks.png", dpi = 1200)
plt.figure()
plt.plot(t, CTE[i,:])
plt.xlabel('tasks')
plt.ylabel('Current Task Error')
plt.savefig("/home/kraghavan/Projects/Continual/MAML/Results_paper/MAML_training_current_tasks.png", dpi = 1200)
np.savetxt('/home/kraghavan/Projects/Continual/MAML/Results_paper/CME_MAML.csv',\
CME, delimiter = ',')
np.savetxt('/home/kraghavan/Projects/Continual/MAML/Results_paper/CTE_MAML.csv',\
CTE, delimiter = ',')
np.savetxt('/home/kraghavan/Projects/Continual/MAML/Results_paper/TE_MAML.csv',\
TE, delimiter = ',')